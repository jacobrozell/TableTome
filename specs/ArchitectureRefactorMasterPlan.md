...
**Status:** Phases 0–8 substantially complete; Phase 9 naming bridge landed; `switch gameSystemId` / raw-id literal drawdown remains for future editions.
...
| **Phase 5: Play UI split** | 5 | `PlayShell`, engine VMs + `BattleTrackerViewModelFactory`, `BattleTrackerSections` → `Features/Play/Shared/`, `GuidedMatchView` ~190 LOC | **506 tests green** | high |
| **Phase 6: DS consolidation** | 6 | `GameGuideStartHereCard` / `GameGuideWhatYouNeedCard` capability routers | build + tests | high |
| **Phase 7: navigation** | 7 | `ActiveGameContextStore` deleted → `ActiveGameContextPersistence`; unified `AppRouter` | build + tests | high |
| **Phase 8: persistence/concurrency** | 8 | `MatchSyncSchemaPolicy`, `HobbySchemaMigrationPlan` scaffold, strict concurrency on Domain/Data | build + tests | high |
| **Phase 9: naming bridge** | 9 | `PlayCatalogNaming.swift` typealiases | build | high |
| Ratchet: `battlerules_or_probes` → **0** | 3 | word-boundary grep; capability-based `WrongGuideResolver` | **ran**: enforce passes | high |

### Remaining (incremental / post-1.0)

- **`switch gameSystemId` → 0** (21 today): migrate UI switches to `playEngine` / capability reads.
- **`raw_id_literals` → 0** (46 today): prefer `GameSystemId` at call sites.
- **Full `Spearhead*` → `Play*` rename** beyond typealiases.
- **SwiftData migration stages** once hobby models change post-1.0.
- **Delete legacy per-system Start Here card files** after snapshot coverage.
...
| Field | Value |
|-------|-------|
| Status | Phases 0–8 landed; Phase 9 typealias bridge; ratchet at 0 for probes + system-caps |
| Baseline metrics (remaining) | `switch gameSystemId`: 21 · raw id literals: 46 |
